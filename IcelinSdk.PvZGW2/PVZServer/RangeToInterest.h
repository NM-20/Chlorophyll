#pragma once

#include <IcelinSdk.PvZGW2/PrimitiveTypes.h>
#include <IcelinSdk.PvZGW2/PVZServer/PVZBTreeBoolProvider.h>
#include <IcelinSdk.PvZGW2/PVZShared/CompareType.h>
#include <IcelinSdk.PvZGW2/PVZShared/ConditionTarget.h>

namespace fb
{

/* TypeInfo (Array): 000000014285EFB8
   RuntimeId:        1D3F
   TypeInfo Kind:    DataContainerClassInfo
   ClassId:          1040
   VfTable:          000000014231DE98
   Address (Base):   000000014310B910
*/
#pragma pack(push, 8)
class RangeToInterest : public PVZBTreeBoolProvider
{
public:
  typedef struct DataContainerClassInfo TypeInfo_t;

  FB_INT32 Interest; /* 0x0010 */
  CompareType CompareType; /* 0x0014 */
  FB_HANDLE(class FloatProvider) Range; /* 0x0018 */
  ConditionTarget Target; /* 0x0020 */
  char pad_0024[0x0004];

private:
  /* Intended for use in Ida. Do not remove. */
  static void IdaForceImport(TypeInfo_t);
}; /* Size: 0x0028 */
#pragma pack(pop)

static_assert(sizeof(RangeToInterest) == 40);

}
