...
template<>
struct SimplifyMemFunc< 12 /* SINGLE_MEMFUNCPTR_SIZE + sizeof(int) */ >  {
	template <class X, class XFuncType, class GenericMemFuncType>
	inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
		GenericMemFuncType &bound_func) { 
...
template <>
struct SimplifyMemFunc< 16 /* SINGLE_MEMFUNCPTR_SIZE + 2*sizeof(int) */ >
{
...
template <>
struct SimplifyMemFunc< 20 /* SINGLE_MEMFUNCPTR_SIZE + 3*sizeof(int) */ >
{
	template <class X, class XFuncType, class GenericMemFuncType>
	inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
		GenericMemFuncType &bound_func) {
...
protected: 
	...
	// HACK: Clang doesn't seem to have the same trick to achieve an MFP with a
	// size of eight bytes, so we'll need to take a different approach.
	// We will only have this issue with Ida, so we'll choose a regular function
	// pointer to achieve eight bytes.
#if defined(IDA64)
